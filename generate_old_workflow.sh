...
if [ ${PLATFORM} == "osgconnect" ] ; then
 ...
 PROJECT_PATH=${BASE}/analysis_old/analysis-${n}-${RUN_TAG}-${UNIQUE_ID}
...
--config-files \
  ...
  ${BASE}/config_old/executables_mod.ini \
  ${BASE}/config_old/plotting_mod.ini \
  ${BASE}/config_old/injections_bns.ini \
--config-overrides ${CONFIG_OVERRIDES} ${PLATFORM_CONFIG_OVERRIDES} \
  ...
  "inspiral:fake-strain:aLIGOZeroDetHighPower" \
  "calculate_psd:fake-strain:aLIGOZeroDetHighPower" \
--found-inj-snr-series
