...
v1_trig_merge_file = glob.glob("{}/V1-HDF_TRIGGER_MERGE_BNSSTT2_INJ*.hdf".format(scratch_dir))[0]
...
v1_snr_files = sorted(glob.glob("{}/V1-SINGLE_TEMPLATE_P1_SNR_SERIES_V1_*.hdf".format(foundinj_dir)))
...
with h5py.File(injfind_file, "r") as fp:
    ...
    v1_trig_ids = fp['found_after_vetoes/V1/trigger_id'][:]
...
for i in range(nfound):
        ...
        coinc_ifos = [ifo.upper() for ifo in ['h1', 'l1', 'v1'] if
        ...
        with h5py.File(bank_file, "r") as fp:
            ...
            hlv_template_stats = {k: v[temp_id] for k, v in fp.items()}
        bank_stats = list(bank_stats) + ['mchirp', 'eta', 'mtotal']
        hlv_template_stats['mchirp'] = mchirp_from_mass1_mass2(hlv_template_stats['mass1'],
                                                              hlv_template_stats['mass2'])
        hlv_template_stats['eta'] = eta_from_mass1_mass2(hlv_template_stats['mass1'],
                                                        hlv_template_stats['mass2'])
        hlv_template_stats['mtotal'] = hlv_template_stats['mass1'] + hlv_template_stats['mass2']
        ...
        with h5py.File(v1_trig_merge_file, "r") as fp:
            v1_tm = {s: fp['V1/{}'.format(s)][v1_trig_id] for s in trig_stats}
        ...
        for ifo in ['h1', 'l1', 'v1']:
            if eval('{}_time'.format(ifo)) != -1.0:
                exec('sim.{}_end_time = int({}_time)'.format(ifo[0], ifo))
                exec('sim.{}_end_time_ns = int({}_time % 1 * 1e9)'.format(ifo[0], ifo))
            else:
               logging.info("{} not in coinc, skipping".format(ifo.upper()))
               #exec('sim.{}_end_time = None'.format(ifo[0]))
               #exec('sim.{}_end_time_ns = None'.format(ifo[0]))
        for s in bank_stats:
            exec("sim.{0} = hlv_template_stats['{0}']".format(s))
        ...
        for i, ifo in enumerate(['h1', 'l1', 'v1']):
            if eval('{}_time'.format(ifo)) != -1.0:
                ...
                for s in bank_stats:
                    exec("{0}_row.{1} = hlv_template_stats['{1}']".format(ifo, s))
